...
public class VerificationService {
    ...
    public VerifyPresentationResponse verifyPresentation(VerifyPresentationRequest request) {
        ...
        boolean stateMatches = verifierStateService.isActiveAuthorizationState(requestState);
        if (!stateMatches) {
        ...
        try {
            ...
            if (walletReportedMissingCredential(decoded) || !hasCredentials(presentation)) {
                verifierStateService.clearVerifiedCredential();
                verifierStateService.publishVerifierError("Wallet has no verifiable credential to satisfy the request. Please issue the credential first.");
            ...
            if (!challengeMatches(presentation, request.getChallenge())) {
                verifierStateService.clearVerifiedCredential();
                verifierStateService.showVerifierQr();
                verifierStateService.publishVerifierError("Presentation challenge does not match the verifier request.");
            ...
            if (!submissionMatchesDefinition(request)) {
                verifierStateService.clearVerifiedCredential();
                verifierStateService.showVerifierQr();
                verifierStateService.publishVerifierError("Presentation submission mapping does not satisfy the definition requirements.");
            ...
            verifierStateService.recordVerifiedCredential(preview);
            ...
            verifierStateService.clearVerifiedCredential();
            verifierStateService.showVerifierQr();
            verifierStateService.publishVerifierError("Invalid presentation encoding: " + ex.getMessage());
            return new VerifyPresentationResponse(false, null, "Invalid presentation encoding: " + ex.getMessage());
        } catch (Exception ex) {
            verifierStateService.clearVerifiedCredential();
            verifierStateService.showVerifierQr();
            verifierStateService.publishVerifierError("Failed to process presentation: " + ex.getMessage());
