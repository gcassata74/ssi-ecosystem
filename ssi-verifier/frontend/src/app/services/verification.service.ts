...
export class VerificationService implements OnDestroy {
  private readonly updatesSubject = new Subject<VerifierQr>();
  ...
  fetchCurrent(): Observable<VerifierQr> {
    const params = this.buildQueryParams();
    return this.http.get<unknown>('/api/verification/qr', { params }).pipe(
      map(payload => this.normalizePayload(payload))
    );
  }

  updates(): Observable<VerifierQr> {
  ...
  private subscribeToTopic(): void {
    ...
    this.clearSubscription();
    this.subscription = this.client.subscribe('/topic/verification', message => {
  ...
  private handleMessage(message: IMessage): void {
    ...
    try {
      ...
      console.error('Unable to parse verification update', error);
    }
  }

  private normalizePayload(payload: unknown): VerifierQr {
    ...
    if (candidate['verifier'] || candidate['currentStep']) {
  ...
  private normalizeStatusPayload(status: Record<string, unknown>): VerifierQr {
    ...
    throw new Error('Unsupported verification status payload');
  }
  ...
  private normalizeQr(source: Record<string, unknown>): VerifierQr {
